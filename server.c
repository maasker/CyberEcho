// Standard input/output header for basic input and output functions.
#include <stdio.h>
// Standard library header for various functions including memory allocation, process control, conversions, etc.
#include <stdlib.h>
// String header for various string handling functions.
#include <string.h>
// Unistd.h is the name of the header file that provides access to the POSIX operating system API. It stands for "UNIX standard."
#include <unistd.h>
// Header file for the socket API, used for network communication.
#include <sys/socket.h>
// Standard boolean library for using 'true' and 'false' macros.
#include <stdbool.h>
// Limits header provides characteristic values of integral types for the specific system and compiler implementation used.
#include <limits.h>
// Header for definitions of structures needed for internet domain addresses.
#include <arpa/inet.h>
// Pthread.h is a header for POSIX thread programming. It is used to create and manage threads.
#include <pthread.h>
// Custom header file for defining structures and functions related to the chat application.
#include "chat.h"
...
int main() {
    // Initialize server and load user data
    startup();
    ...
    // Server loop: waiting for connections
    while (true) {
...
char* getOnlineUserList(int currentClientSocketId)
{
    ...
    struct User* currentUser = (struct User*)userList;
    memset(onlineUser, '\0', MAX_SIZE * sizeof(char));
    while (currentUser != NULL)
    {
        char sss[1000];
